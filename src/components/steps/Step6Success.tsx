...
// ─── Floating particle halo behind the checkmark ───────────────────────────
...
// ─── Inline SVG checkmark (no font dependency) ─────────────────────────────
function CheckIcon() {
  return (
    <svg
      viewBox="0 0 24 24"
...
// ─── Stable ref ID derived from form data ──────────────────────────────────
function buildRefId(formData: FormData): string {
  const seed =
    (formData.mobileNumber + formData.panNumber).replace(/\D/g, '').slice(-6) || '987654';
  return `FM-${seed}`;
}
...
export default function Step6Success({ formData, onExit }: Step6SuccessProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto w-full text-center py-16 px-4">
      ...
      {/* Visually hidden escape hatch — keeps onExit prop wired */}
      <button
        onClick={onExit}
